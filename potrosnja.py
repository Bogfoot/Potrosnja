...
def newPriceCalculation(df, brojProizvoda = 0):
    ...
    for item in range(len(df['Cijena'])):
        if df['Valuta'][item] != native_val:
            df['Cijena'][item] = c.convert(df['Cijena'][item],str(df['Valuta'][item]).upper(),str(native_val).upper())
...
def showPlots(df):
    ...
    fig.add_trace(go.Line(x = df['Datum'], y = df['Kumulativna suma'], name = "Kumulativna potrošnja"))
